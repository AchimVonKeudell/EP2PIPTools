...
KTRAP_mol = 2.7e4 # Rate constant of loss reaction of H2O2 in liquid after source ist abgeschaltet in m^3/(mol*s)
KTRAP = KTRAP_mol/(6e23) # Rate constant of loss reaction of H2O2 in liquid after source ist abgeschaltet
...
def apply_boundary_conditions(conc: np.ndarray, concTrap: np.ndarray, gp: int) -> None:
    conc[:gp] = conc[gp]
    conc[-gp:] = conc[-gp - 1]
    concTrap[:gp] = concTrap[gp]
    
def compute_derivative(conc: np.ndarray, concTrap: np.ndarray, diff: float, dz: float,
                        flux: float, gp: int, velocity: float, reaction_on: bool = True, 
    ...
    for i in range(gp, len(conc) - gp):
        ...
        if reaction_on:
            reaction = -conc[i]*KTRAP*concTrap[i]
        else:
            reaction = 0.0

        deriv[i] = diffusion + convection + inflow + reaction
    return deriv
...
def run_simulation(diff: float, velocity: float,
    ...
    for step in tqdm(range(n_steps), desc=f"Simulation (u={velocity:.2e} m/s)"):
        ...
        while t < DT_PLOT:
            ...
            reaction_on = not influx_on
            # t_global = step * DT_PLOT + t

            # influx_on = t_global < SOURCE_OFF
            # reaction_on = t_global >= SOURCE_OFF
            deriv = compute_derivative(conc, concTrap, diff, DZ, J_H2O2, GP,
                                        velocity, reaction_on, influx_on)
            euler_step(conc, deriv, DT0, DZ, diff)
            # concTrap[:] = conc
            
            apply_boundary_conditions(conc, concTrap, GP)
            t += DT0
...
if __name__ == "__main__":
    ...
    animate_profiles(results_diff, save_path="diffusion_loss.mp4", fps=15)    
    animate_profiles(results_conv, save_path="convection_diffusion_loss.mp4", fps=15)
    plot_comparison(results_diff, results_conv, save_path="comparison_loss.png")
