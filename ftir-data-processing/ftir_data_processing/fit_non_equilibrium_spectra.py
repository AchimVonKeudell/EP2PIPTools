...
from .vibrationalspectrum.fit import fit_non_equilibrium_spectrum
...
def process_spectra(metadata: dict, processing_step: str):
    ...
    def fit_spectrum(input_file_name: str):
        ...
        output_files = [f"{output_directory_fits}\\{fn_index}.{file_type}" for file_type in EXTENSIONS]

        if os.path.exists(output_files[0]):
            print(f'skip {output_files[0]}')
            return  # already done, not redoing work ;p

        fit_non_equilibrium_spectrum(
    ...
    for parameter in [f'fraction_{actual_molecule_name(processing_step).lower()}',
                      ...
                      'temperature_vibration_kelvin',
                      'temperature_vibration_12_kelvin',
                      'temperature_vibration_3_kelvin']:
